...
class StyleExtHtmlWebpackPlugin5 {
  constructor(options) {
  ...
  apply (compiler) {
    ...
    if (compiler.hooks) {
      ...
      compiler.hooks.emit.tapAsync('StyleExtHtmlWebpackPlugin5', emit);
    } else {
  ...
  wirePluginEvent (event, compilation, fn) {
    ...
    if (compilation.hooks) {
      if (compilation.hooks[event]) {
        ...
        compilation.hooks[event].tapAsync('StyleExtHtmlWebpackPlugin5', wrappedFn);
      } else {
        const hooks = HtmlWebpackPlugin.getHooks(compilation);
        hooks[htmlWebpackHookEvents[event]].tapAsync('StyleExtHtmlWebpackPlugin5', wrappedFn);
      }
    } else {
...
module.exports = StyleExtHtmlWebpackPlugin5;
